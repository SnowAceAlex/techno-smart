    ...
    <div className="w-full bg-white px-5 pt-5 pb-2">
      ...
      <RadioGroup className="mt-2 space-y-1" value={selectedPrice || ""}>
        {priceArray?.map((price, index) => (
          ...
          >
            ...
            <Label
              htmlFor={price.value}
              className={`${selectedPrice === price?.value ? "font-semibold text-dark_blue" : "font-normal"}`}
            >
              {price?.title}
            </Label>
          </div>
      ...
      {selectedPrice && (
        <button
          ...
          className="text-sm font-medium mt-2 underline underline-offset-2 decoration-1 hover:text-dark_blue hoverEffect"
        >
          Reset selection
        </button>
      )}
